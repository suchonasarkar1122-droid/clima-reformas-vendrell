        ...
        <div className="max-w-[900px] flex flex-col items-center gap-5">
          ...
          {h1Keyword ? (
            <>
              ...
              <h2 className="font-display font-extrabold text-[36px] md:text-[56px] lg:text-[72px] text-white leading-[0.95]">
                {title}
              </h2>
            </>
          ) : (
            <h1 className="font-display font-extrabold text-[36px] md:text-[56px] lg:text-[72px] text-white leading-[0.95]">
              {title}
            </h1>
          )}
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-white/70 text-sm">
              {trustItems.map((item, i) => (
                <span key={i} className="flex items-center gap-1.5">
                  {i === 0 && (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#D89564">
