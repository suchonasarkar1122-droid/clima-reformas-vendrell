...
export default function HeroForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xaqvygaw", {
        method: "POST",
        body: data,
  ...
  if (submitted) {
    return (
      <div className="bg-[#16302A]/95 backdrop-blur-md rounded-2xl p-8 text-center">
        ...
        <h3 className="font-body font-bold text-xl text-white mb-2">¡Recibido, gracias!</h3>
        <p className="font-body text-white/60 text-sm">Te llamamos lo antes posible con tu valoración personalizada.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#16302A]/95 backdrop-blur-md rounded-2xl p-7 md:p-8 shadow-2xl">
      <div className="text-center mb-6">
        <h3 className="font-body font-bold text-xl md:text-2xl text-white mb-1">
          Valoración gratuita
        </h3>
        <p className="font-body text-white/50 text-sm">
          Sin coste ni obligación. Te llamamos lo antes posible.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
        <input type="hidden" name="_subject" value="Nuevo presupuesto desde reformasvendrell.es (hero)" />
        ...
        <div className="relative">
          <select name="locality" required className={selectClasses} defaultValue="">
            ...
            <option value="El Vendrell" className="text-dark">El Vendrell</option>
            <option value="Calafell" className="text-dark">Calafell</option>
            <option value="Cunit" className="text-dark">Cunit</option>
            <option value="L'Arboç" className="text-dark">L&apos;Arboç</option>
            <option value="Roda de Berà" className="text-dark">Roda de Berà</option>
            <option value="Banyeres del Penedès" className="text-dark">Banyeres del Penedès</option>
            <option value="Otra" className="text-dark">Otra población</option>
          </select>
        ...
        <div className="relative">
          <select name="serviceType" className={selectClasses} defaultValue="">
            ...
            <option value="Piso costero" className="text-dark">Piso costero</option>
            <option value="Casa de pueblo" className="text-dark">Casa de pueblo</option>
        ...
        <label className="flex items-start gap-2 text-white/60 text-xs">
          ...
          <span>
            Acepto la <a href="/politica-privacidad" target="_blank" rel="noopener noreferrer" className="underline hover:text-white">Política de Privacidad</a>
          </span>
        </label>

        {error && (
          <p className="text-red-400 text-xs text-center">No hemos podido enviar el formulario. Prueba de nuevo o llámanos al 877 278 163.</p>
        )}
        ...
        <div className="flex gap-2.5 mt-1">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-accent text-white py-4 rounded-full font-body font-bold text-[15px] hover:bg-accent-light transition-colors inline-flex items-center justify-center gap-2.5 disabled:opacity-60"
          >
            {loading ? (
        ...
        <p className="text-center text-white/30 text-xs flex items-center justify-center gap-1.5 mt-1">
          ...
          Tus datos quedan protegidos. Sin spam.
        </p>
      </form>
    </div>
  );
