...
export default function CTAForm({ defaultServiceType = "" }: CTAFormProps) {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-white/90">
          Te llamamos lo antes posible con la valoración a medida de tu obra.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <input type="hidden" name="_subject" value="Nuevo presupuesto desde reformasvendrell.es (CTA)" />
      ...
      <div>
        ...
        <select name="serviceType" defaultValue={defaultServiceType} required className="w-full px-4 py-3 rounded-xl bg-white/20 text-white border-none focus:outline-none focus:ring-2 focus:ring-white/40">
          ...
          <option value="Piso costero" className="text-dark">Piso costero (Coma-ruga / Sant Salvador)</option>
      ...
      <div className="flex items-start gap-2">
        ...
        <label htmlFor="lopd" className="text-white/70 text-xs leading-relaxed">
          Acepto la <a href="/politica-privacidad" className="underline text-white/90">política de privacidad</a> y el tratamiento de mis datos para recibir presupuesto.
        </label>
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-dark text-white py-4 rounded-xl font-bold uppercase tracking-wider hover:bg-[#142822] transition-colors text-[15px] disabled:opacity-60 inline-flex items-center justify-center gap-2"
      >
        {loading && <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
        {loading ? "Enviando..." : "Pedir mi valoración gratuita"}
      </button>
